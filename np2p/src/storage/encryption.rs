...
///
/// The `nonce_context` must be unique per (file, key, segment) to avoid nonce reuse.
/// Callers should pass something like the content hash, file id, or a
/// (key || segment-id) binding. Using a deterministic nonce ensures re-encryption
/// with the same inputs produces the same ciphertext, so a repaired shard is
/// always compatible with surviving shards.
pub fn encrypt(data: &[u8], key_bytes: &[u8], nonce_context: &[u8]) -> Result<Vec<u8>> {
    ...
    // Derive nonce deterministically from (key || context) so re-encryption always
    // yields the same ciphertext. Including the key means a nonce_context shared
    // across two different keys can never cause nonce reuse.
    let nonce_hash = blake3::hash([key_bytes, nonce_context].concat().as_slice());
