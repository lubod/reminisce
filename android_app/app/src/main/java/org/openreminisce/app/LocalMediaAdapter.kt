...
class LocalMediaAdapter(
    ...
    private val onMediaClick: (ImageInfo) -> Unit
    ...
    inner class MediaViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        private val mediaTypeIcon: ImageView = view.findViewById(R.id.mediaTypeIcon)
        private val backupStatusBadge: ImageView = view.findViewById(R.id.backupStatusBadge)

        fun bind(mediaItem: MediaItem.Image) {
            val imageInfo = mediaItem.imageInfo

            // Load thumbnail from local URI using Glide
            val contentUri = Uri.parse(imageInfo.id)
            ...
            if (imageInfo.mediaType == "video") {
                mediaTypeIcon.setImageResource(R.drawable.ic_play_circle)
                mediaTypeIcon.visibility = View.VISIBLE
            } else {
                mediaTypeIcon.visibility = View.GONE
            }

            // Show backup status badge
            if (imageInfo.isBackedUp) {
                backupStatusBadge.setImageResource(R.drawable.ic_check_circle)
                backupStatusBadge.visibility = View.VISIBLE
            } else {
                backupStatusBadge.visibility = View.GONE
            }

            thumbnailView.setOnClickListener {
                onMediaClick(imageInfo)
            }
        }
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (val item = items[position]) {
            ...
            is MediaItem.Image -> (holder as MediaViewHolder).bind(item)
        }
    }
