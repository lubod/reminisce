...
- [ai_client.rs](file:///Users/ldr/work/reminisce/src/ai_client.rs): Shared gRPC client (`tonic`); a process-wide `AiClient::shared(&config)` reuses one `Channel` (the per-instance `OnceCell` is no longer recreated per RPC). `is_permanent_failure()` classifies tonic statuses (InvalidArgument/Unauthenticated/…) so workers only skip genuinely permanent errors.
