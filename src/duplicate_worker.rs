...
async fn process_batch(
    ...
    let total_images: i64 = counts.get(0);
    let unchecked: i64 = counts.get(1);

    // Count stored pairs (best-effort; failures are non-fatal)
    let pair_count: i64 = client
    ...
    if unchecked == 0 {
        ...
        // Return Ok(false) → run_worker_loop backs off to max_interval (300 s)
        return Ok(false);
    }
    ...
    // Return Ok(true) → run_worker_loop sleeps min_interval (200 ms) then calls us again
    Ok(true)
}
